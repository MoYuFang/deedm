...
let tot_vertex = 10
function add_vertex() {
    graph.add_vertex(`${tot_vertex}`); tot_vertex+=1;
}

function del_vertex() {
    graph.del_vertex(`${tot_vertex-1}`); tot_vertex-=1;
}
...
onMounted(() => {
    ...
    // for (let i = 0; i < tot_vertex; ++i) graph.add_vertex(`${i}`);
...
<template>
    ...
    <div id="container">
        ...
        <!-- <div>
            <button @click="add_vertex">add</button>
            <button @click="del_vertex">del</button>
        </div> -->
    </div>
